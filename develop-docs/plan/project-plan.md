...
- **重构加载逻辑**: 表单加载时，根据 `jobConfig` 中的 `datasourceId` 自动重新查询数据源详情并填充表单。通过 `async/await` 保证数据加载先于回显渲染。

#### 2. ✅ Doris Sink 高级选项增强 (已完成)
...
**测试步骤**:
1. 运行一个需要较长时间的 SeaTunnel 任务 (通过 `pg_sleep()` 实现)。
