...
- ✅ 后端数据源逻辑重构
- ✅ Doris Sink 高级选项增强
- ✅ Source 高级选项增强（`parallelism`、`partition_column`、`split.size`、`fetch_size`）
- ✅ 参数化增强（项目级别参数 + 自定义参数）
- ✅ 任务停止功能测试
- ✅ UI/UX 优化（通用输入框半宽、区块背景色、删除按钮位置、JSON 预览高度）
- ✅ 任务分组与图标配置（移至"通用组件"分组，添加自定义图标）
- ✅ Transform 输入选项优化（避免循环引用）
- ✅ 文档更新与同步
...
**最后更新**: 2025-10-27
**变更记录**:
- v1.6 (2025-10-27): 更新项目当前状态，补充 Source 高级选项、UI/UX 优化、任务分组与图标配置、Transform 循环引用修复等已完成功能。
